...
minetest.register_chatcommand("lua2mts", {
	...
	func = function(name, param)
		...
		if not file then
			return false, S("Lua file not found.")
		end

		local content = file:read("*all")--.." return(schematic2)"
		file:close()

		local load_func, err = loadstring("schematic2 "..content)
		if not load_func then
			return false, S("Failed to load Lua file.")
		end
		...
		if not success or type(schematic2) ~= "table" then
			...
			minetest.log("schematic2:", "schematic2:" .. tostring(schematic2))
		...
		local schem_lua = lua_file:gsub("%.lua$", "")  -- Remove .lua extension
		local mts_path = export_path_full .. DIR_DELIM .. schem_lua .. ".mts"
		local mts_file, err = io.open(mts_path, "wb")
...
minetest.register_chatcommand("lua2mts", {
	...
	func = function(name, param)
		...
		local load_func, err = loadstring("return " .. content)
		if not load_func then
			return false, S("Failed to load Lua file.")
		end
...
minetest.register_chatcommand("lua2mts", {
	...
	func = function(name, param)
		...
		local lua_path = export_path_full .. DIR_DELIM .. lua_file
		--local load_func, err = loadfile(lua_path)
		local load_func = dofile(lua_path)
	--	if not load_func then
	--		return false, S("Failed to load Lua file: ") .. err
	--	end
		minetest.log("schem2:", "schem2table:")
		logTable(load_func)
		local success, schematic2 = pcall(load_func)
		minetest.log("success:", "success:" .. tostring(success))
		logTable(schematic2)
...
local mts_save1 = function(name, schematic)
	local s = minetest.serialize_schematic(schematic, "mts", {})
...
-- [chatcommand] Convert .lua file to MTS schematic file
minetest.register_chatcommand("lua2mtslf", {
	description = S("Convert .lua file to .mts schematic file"),
	privs = {server = true},
	params = S("<lua file>"),
	func = function(name, param)
		...
		print("hello1")
		minetest.log("hello2", "hello2")

		return true, S("Exported schematic to " .. lua_file)
	end,
})
...
minetest.register_chatcommand("lua2mts", {
	...
	func = function(name, param)
		...
		local schem_table = minetest.read_schematic(lua_path, {})
		logSchematicTable(schem_table)
		-- Sanity check - is this a minetest schematic?
		minetest.log("hello","hello")
		logTable(schem_table)
		if not schem_table or not schem_table.size or not schem_table.data then
			return false, S("Invalid schematic data in Lua file.")
		end

		minetest.log("success:", "success:" .. tostring(true))
		logTable(schem_table)
		--mts_save(lua_file, schem_table)
		--return true, S("Exported schematic to @1", mts_path)
	end,
})
...
--[[minetest.register_chatcommand("lua2mts", {
	--description = S("Convert .lua file to .mts schematic file"),
	description = ("Convert .lua file to .mts schematic file"),
	privs = {server = true},
	--params = S("<lua file>"),
	params = ("<lua file>"),
	func = function(name, param)
		local f = assert(io.open(current_path_location .. param, "r"))
